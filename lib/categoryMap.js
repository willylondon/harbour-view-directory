...
const DEFAULT_CATEGORY = CATEGORY_STYLES['General Services'];
...
const MARINE_KEYWORDS = ['fishing', 'bait', 'tackle', 'marine supplies', 'marine', 'fishing supplies', 'fishing gear', 'fishing rods', 'boat supplies', 'boat'];
const FINANCE_KEYWORDS = ['atm', 'bank', 'credit union', 'cambio', 'remittance', 'western union', 'moneygram', 'loan', 'bill payment', 'bill pay', 'bill express', 'bills', 'money transfer'];
const HEALTH_KEYWORDS = ['clinic', 'medical', 'health', 'physician', 'pharmacy', 'dentist', 'nurse', 'patient', 'surgery', 'hospital', 'lab', 'health centre', 'health center', "doctor's office", 'doctor’s office', 'doctor office'];
...
const EDUCATION_KEYWORDS = ['teacher', 'tutor', 'tutoring', 'school', 'class', 'lessons', 'education', 'academy', 'learning', 'csec', 'cape', 'homework', 'reading', 'maths', 'math', 'english', 'basic school', 'prep school', 'college', 'learning centre', 'learning center'];
...
const BOOKS_KEYWORDS = ['book', 'books', 'bookstore', 'book shop', 'stationery', 'school supplies', 'office supplies', 'notebook', 'notebooks', 'pens'];
const PETS_KEYWORDS = ['pet', 'pets', 'animals', 'pet shop', 'pet supplies', 'exotic pets', 'mobile zoo', 'birds', 'parrot', 'rabbit', 'veterinary', 'aquarium', 'dog food'];
...
export const SEARCH_SYNONYMS = {
  food: ['lunch', 'dinner', 'breakfast', 'cook shop', 'takeout'],
  lunch: ['food', 'cook shop', 'takeout', 'patty', 'fried chicken'],
  'fry chicken': ['fried chicken', 'chicken', 'food'],
  'fried chicken': ['fry chicken', 'chicken', 'food'],
  chinese: ['restaurant', 'food'],
  patty: ['patties', 'bakery', 'food'],
  bakery: ['cake', 'cakes', 'pastries', 'patty'],
  gas: ['cooking gas', 'propane', 'cylinder'],
  'cooking gas': ['gas supplier', 'tuna gas', 'vespak'],
  jp: ['justice of the peace'],
  atm: ['bank', 'cash', 'bill payment'],
  bills: ['bill payment', 'bill express', 'money transfer', 'finance'],
  'bill payment': ['bills', 'bill express', 'atm', 'finance'],
  'money transfer': ['moneygram', 'western union', 'remittance', 'finance'],
  pharmacy: ['medicine', 'health', 'drug store'],
  laundry: ['laundromat', 'dry cleaning', 'wash and fold'],
  mechanic: ['auto', 'tyre', 'tire', 'battery', 'parts'],
  ac: ['air conditioning', 'air conditi', 'ac technician'],
  ...
  stationery: ['books', 'school supplies', 'notebooks', 'pens'],
  pets: ['animals', 'exotic pets', 'mobile zoo'],
  pet: ['animals', 'exotic pets', 'mobile zoo'],
  'pet shop': ['pet supplies', 'pets', 'animals'],
  fishing: ['fishing supplies', 'marine supplies', 'bait', 'tackle'],
...
function hasEducationContext(haystack) {
  return includesAny(haystack, EDUCATION_KEYWORDS);
}

function hasMedicalContext(haystack) {
  return includesAny(haystack, HEALTH_KEYWORDS);
}
...
function inferCategoryLabel(vendor) {
  ...
  if (hasEducationContext(haystack)) return 'Education';
  if (hasMedicalContext(haystack)) return 'Health & Medical';
